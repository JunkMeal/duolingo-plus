#!/usr/bin/env node
;
var answers, count, createAccount, i, j, link, ref, spinner;
...
createAccount = async function(referral) {
  var chance, duo, id, jwt, mur, referral_code;
  chance = new Chance();
  duo = axios.create({
  ...
  jwt = setCookie.parse(mur).find(function(e) {
    return e.name === "jwt_token";
  });
  id = mur.data.id;
  return (await duo({
    method: "PATCH",
...
console.log(`${chalk.hex("#58cc02")("Duo")}${chalk.white("lingo ")} ${chalk.hex("#4b4b4b")("plus")} ${chalk.hex("#4b4b4b")("made  by ")} ${chalk.white("JunkMeal")}`);
...
answers = (await inquirer.prompt([
  ...
  },
  {
    ...
    validate: function(input) {
      return !isNaN(input) || "You need to provide a number.";
    },
    default: function() {
      return "19";
    }
...
spinner = ora(`Creating accounts (0/${count})`).start();
